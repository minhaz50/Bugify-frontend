...
export default function LoginPage() {
  ...
  const fillDemo = (role: 'contributor' | 'maintainer') => {
    const creds = {
      contributor: { email: 'alice@devpulse.com', password: 'password123' },
      maintainer: { email: 'admin@devpulse.com', password: 'password123' },
    };
    setForm(creds[role]);
    setErrors({});
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 dark:from-slate-950 dark:via-slate-900 dark:to-indigo-950 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2">Sign in to your DevPulse account</p>
        </div>

        {/* Demo buttons */}
        <div className="card p-4 mb-4">
          <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">Demo Login</p>
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => fillDemo('contributor')} className="btn-secondary btn-sm text-xs justify-center">
              👤 Contributor
            </button>
            <button onClick={() => fillDemo('maintainer')} className="btn-secondary btn-sm text-xs justify-center">
              🔧 Maintainer
            </button>
          </div>
        </div>
        ...
        <div className="card p-8">
          ...
          <p className="text-center text-sm text-slate-500 dark:text-slate-400 mt-6">
            ...
            <Link href="/auth/signup" className="text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 font-semibold">Create account</Link>
          </p>
        </div>
      </div>
    </div>
